...
- Access the package vignette(s).
  How many vignette does the package include? - ++5++ 
...
- What are the metadata columns available in the object? ++source, type, score, phase, gene_id, gene_name++

```{r}
names(mcols(gtf_data))
```
...
```{r}
assay_counts <- read_csv("/project/shared/r/1_r_data_science/8-bioconductor/summarized_experiment/counts.csv")
assay_counts <- column_to_rownames(assay_counts, var = "...1")
